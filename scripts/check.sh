...
# the staged index, the identity the next commit would carry, and every commit
# this fork authored — metadata, message and tree — so it needs full history and
# fails on a shallow clone. The check itself lives in its own script, which the
# CI workflow runs too, so the pattern, the allowlist and the commit range have
