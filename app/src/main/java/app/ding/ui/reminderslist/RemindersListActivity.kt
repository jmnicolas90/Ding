...
import app.ding.BootReceiver
import app.ding.BuildConfig
...
import app.ding.ui.actions.DisplayWelcomeMessage
import app.ding.ui.actions.DisplayWelcomeMessageUpdate
import app.ding.ui.util.HtmlDialogFragment
import app.ding.ui.util.UIUtils
import app.ding.util.AlarmManagerUtil
import app.ding.util.ImplementationError
...
class RemindersListActivity : AppCompatActivity() {
    ...
    /**
     * On Android 12 and 12L, check whether exact alarms may be scheduled. From API 33 on this is
     * not needed, as USE_EXACT_ALARM is non-revocable.
     */
    private fun checkScheduleExactPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU
            || Prefs.isScheduleExactPermissionDontShowAgain(this)
            || AlarmManagerUtil.canScheduleExact(this)
        ) return
