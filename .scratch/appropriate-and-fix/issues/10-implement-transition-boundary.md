...
- **(high) `setOnlyAlertOnce(true)` on every notification kind silenced nagging** — fixed. Only-alert-once keeps Android quiet when a notification replaces one still on screen, which is exactly what a nag does, so a nag re-posted under the reminder's id made no sound. The two sound decisions now live in `app/src/main/java/app/ding/state/NotificationAlerting.kt` as `notificationAlerts` and `notificationAlertsOnlyOnce`, plain functions the builder calls: a nag no longer sets only-alert-once, a delivery keeps it as defence in depth, a re-show stays silent. Covered by `NotificationAlertingTest`.
- **(medium) An alarm set before the `Notify` → `Deliver` rename crashed the broadcast receiver** — fixed. The old payload names the subtype `app.ding.ReminderManager.ReminderAction.Notify`, which the new code could not decode, so `fromJson` threw inside `onReceive`. The subtype names are now pinned with `@SerialName` so they no longer follow the class name, and the decoder reads the old names, translating `Notify` to `Deliver`. The expected due time is optional, and an alarm that carries none delivers when the reminder is still `SCHEDULED` and past due (nags when it is still `NOTIFIED` and nagging) rather than being dropped as stale. A payload that still cannot be read is answered with null, and `processReminderAction` logs it and runs Reconcile instead of throwing. Covered by `ReminderActionTest` and four cold-start tests in `ReminderCommandRunnerTest`; the rule is written into `docs/reminder-state-machine.md`.
